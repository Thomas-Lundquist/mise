...
globalThis.window = { localStorage: localStore, sessionStorage: fakeStorage() };
...
check("picks localStorage when available", S.getStorageTier(), "local");

// --- Legacy purge ----------------------------------------------------------
localStore.setItem("mise-planner:untitled", '{"old":"shape"}');
localStore.setItem("mise-planner:chicken-piccata", '{"old":"shape"}');
localStore.setItem("mise-planner:timer", '{"minutes":10}');
localStore.setItem("unrelated-app-key", "keep me");
check("purges old single-slot drafts", S.purgeLegacy(), 2);
check("but keeps the timer", localStore.getItem("mise-planner:timer") !== null, true);
check("and doesn't touch other apps", localStore.getItem("unrelated-app-key"), "keep me");
...
S.savePlan(stale);
const raw = JSON.parse(localStore.getItem(`mise-planner:plan:${stale.id}`));
raw.version = 1;
localStore.setItem(`mise-planner:plan:${stale.id}`, JSON.stringify(raw));
...
const planKeys = [...localStore._map.keys()].filter((k) => k.startsWith("mise-planner:plan:"));
